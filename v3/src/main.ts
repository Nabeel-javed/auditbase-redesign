...
import { initGlass, type Glass } from './glass'
...
try {
  if (!RM) glass = initGlass(document.getElementById('glass') as HTMLCanvasElement)
} catch (err) { console.warn('glass init failed', err); glass = null }
if (!glass) document.body.classList.add('no-gl')
